...
class nn_linear_layer:
    ...
    ## Q1
    def forward(self,x):
        fwd = np.dot(x,self.W.T) + self.b.T
        return fwd
    ...
    def backprop(self,x,dLdy):
        ...
        dydb = np.ones((x.shape[0], 1))
        dLdb = np.dot(dydb.T, dLdy)
...
class nn_activation_layer:
    ...
    ## Q4
    def backprop(self,x,dLdy):
        dx = dLdy * (1 - self.sigmoid) * self.sigmoid
        return dx
...
class nn_softmax_layer:
    ...
    def forward(self,x):
        exp = np.exp(x)
        sum_of_exp = np.sum(exp, axis=1, keepdims=True)
...
class nn_cross_entropy_layer:
    ...
    def forward(self,x,y):
        ...
        one_hot[range(batch_size), y_train.reshape(batch_size,)]=1
        ce_i = np.max( (-log_of_x) * one_hot, axis=1)
        return np.mean(ce_i)
